...
public class ViewDragHelper {
    ...
    public boolean shouldInterceptTouchEvent(MotionEvent ev) {
        ...
        // 判断是否是向右划动，非向右向直接返回false，下面不作处理
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                float curX = ev.getX();
                float curY = ev.getY();
//			    Log.i("wg", "initX="+initX+"_curX="+curX);
                if (curX < initX) {
                    // 非向右划
                    return false;
                }

                if (curY > initY + OFFSET_Y || curY < initY - OFFSET_Y) {
                    // 当前滑动Y轴的和点击时的Y轴+可偏移的值还大或小，表示用户可能是在上滑或下滑界面内元素而已，所以下面逻辑不需要再处理
                    return false;
                }
                break;
        }
    ...
    private int getEdgeTouched(int x, int y) {
        ...
        if (mTrackingEdges == SwipeBackLayout.EDGE_ALL) {
            result = EDGE_LEFT;
        } else {
