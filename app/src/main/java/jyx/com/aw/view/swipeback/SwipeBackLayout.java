...
public class SwipeBackLayout extends FrameLayout {
    ...
    private View[] mCannotSwipeBackViews;
    private boolean isViewCannotSwipeBack = false;
    ...
    public interface SimpleSwipeListener {
        void onSwipe(float slideOffset);
        void onFinish();
    }
    ...
    public void setCanotSwipeBackViews(View... cannotSwipeBackViews) {
        this.mCannotSwipeBackViews = cannotSwipeBackViews;
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        if (isViewCannotSwipeBack && (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL)) {
            // 初始化还原
            isViewCannotSwipeBack = false;
            return false;
        }

        if (isViewCannotSwipeBack || action == MotionEvent.ACTION_DOWN
                && isTouchCannotSwipeBackView(event.getRawX(), event.getRawY())) {
            isViewCannotSwipeBack = true;
            return false;
        }
    ...
    private boolean isTouchCannotSwipeBackView(float x, float y){
        if (null == mCannotSwipeBackViews || mCannotSwipeBackViews.length == 0) {
            return false;
        }

        int[] location = new int[2];
        int width, height;
        for (View v : mCannotSwipeBackViews) {
            v.getLocationOnScreen(location);
            width = v.getWidth();
            height = v.getHeight();
            if (location[0] <= x && x <= (location[0] + width)
                    && location[1] <= y && y <= (location[1] + height)) {
                return true;
            }
        }
    ...
    private class ViewDragCallback extends ViewDragHelper.Callback {
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
            ...
            // 回调滑动比例0 - 1
            if (null != mSimpleSwipeListener) {
                mSimpleSwipeListener.onSwipe(mScrollPercent);
            }

            if (mScrollPercent >= 1 && !mActivity.isFinishing()) {
                // 回调已经结束
                if (null != mSimpleSwipeListener) {
                    mSimpleSwipeListener.onFinish();
                }
                mActivity.finish();
            }
        }
    ...
    public void setSimpleSwipeListener(SimpleSwipeListener mSimpleSwipeListener) {
        this.mSimpleSwipeListener = mSimpleSwipeListener;
    }
